...
for astrLine in csv.reader( sys.stdin, csv.excel_tab ):
    if astrLine and astrLine[0].startswith( "#" ):
          continue
    strPMID = astrLine[4].split(":")[1]#sfle.regs( 'pmid:(\d+)', astrLine[4], 1 )
    strTax1, strTax2 = (sfle.regs( 'taxid:(\d+)', strCur, 1 ) for strCur in astrLine[2:4])
    if not strTax1 or ( strTax1 != strTax2 ):
        strTax1 = "0"
    if setTaxa and ( strTax1 not in setTaxa ):
        continue
    hashhashPMTaxa.setdefault( strPMID, {} ).setdefault( strTax1, [] ).append( [symbol( hashSymbols, strCur ) for strCur in astrLine] )
...
for strPMID, hashTaxa in hashhashPMTaxa.items( ):
    for strTaxon, aaiLines in hashTaxa.items( ):
        ...
        #strBin = strTaxid + ( "" if ( ( not strPMID ) or ( len( aaiLines ) < iMin ) ) else \
                #        ( "_" + c_strPMID + strPMID ) )
        strBin = strTaxid +  "_" + c_strPMID + strPMID 
