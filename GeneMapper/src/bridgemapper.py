...
if len( sys.argv ) != 7:
        raise Exception( "Usage: bridgemapper.py <inputfile.dat> <outputfile.dat> <mappingfile> <columnlist in inputfile to be mapped> <input system code for bridgemapper> <output system code for bridgemapper>" )
...
c_columnToMap = c_columnToMap[1:-1]
c_columnToMap = filter (lambda a: a != " ", c_columnToMap)
c_columnToMap = filter (lambda a: a != ",", c_columnToMap)
c_columnToMap = filter (lambda a: a != ";", c_columnToMap)
...
#c_mappingfile = sfle.d(c_path_GeneMapper, sfle.c_strDirEtc, c_mappingfilein) #os.path.dirname(c_mappingfile)
c_mappingfile = c_mappingfilein #TODO
...
def convertGeneIds (geneids, mapfile):
    ...
    sfle.ex([c_path_geneidmapper, " -i ", inputfile, " -is",c_origGeneId , " -os ",c_destGeneId, " -o ",outputfile, c_mappingflag , mapfile ," -mm "])
